...
COHERENT = [
    ...
    (
        ...
        "borrower rather than throughput.",
        "well-being",
    ),
    (
...
def _metric(coherent: bool, keyword: str) -> CoherenceMetric:
    ...
    return CoherenceMetric(
        constitution=CONSTITUTION, threshold=0.85, judge=j, samples=1 if j else 5
    )


@pytest.mark.parametrize("name, artifact, keyword", COHERENT, ids=[s[0] for s in COHERENT])
...
@pytest.mark.parametrize("name, artifact, keyword", INCOHERENT, ids=[s[0] for s in INCOHERENT])
