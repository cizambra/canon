...
CASES = [
    ("Wells Fargo — fake accounts", "incoherent",
    ...
    ("Enron — accounting fraud", "incoherent",
    ...
    ("Volkswagen — Dieselgate", "incoherent",
    ...
    ("Purdue Pharma — OxyContin", "incoherent",
    ...
    ("Patagonia — Don't Buy This Jacket", "coherent",
    ...
    ("CVS Health — dropping tobacco", "coherent",
...
def main() -> None:
    ...
    for name, expected, mission, principles, artifact in CASES:
        ...
        res = CoherenceMetric(constitution=con, threshold=THRESHOLD, judge=judge,
                              samples=SAMPLES).score(
            artifact,
            task="judge whether this documented behavior coheres with the organization's own stated values")
        ...
        print(f"{name:34}{expected:12}{res.score:>7.2f} {str(res.gated):>6}  "
