...
def _project(tmp_path, judge_choice_all="yes"):
    (tmp_path / "constitution.yaml").write_text("mission: Serve well\nprinciples:\n  - Be fair\nversion: v1\n")
    (tmp_path / "suite.yaml").write_text(yaml.safe_dump(
        {"task": "decide", "artifacts": ["a fair decision serving the mission"]}))
...
def test_accept_then_check_catches_a_solid_per_question_flip(tmp_path, monkeypatch):
    ...
    def script(q, choices):
        ...
        if flipped["on"] and "value-relevant IMPACT" in q: return "no"   # A2 flips
        return "yes"

    monkeypatch.setattr(cli, "_metric", lambda path, thr: CoherenceMetric(
        constitution=constitution_from_file(path), threshold=thr,
...
def test_cli_uses_the_config_file_named_by_the_config_option(tmp_path, monkeypatch):
    ...
    assert runner.invoke(app, ["accept", "--suite", "suite.yaml",
                               "--config", str(cfg)]).exit_code == 0
    ...
    assert runner.invoke(app, ["check", "--suite", "suite.yaml",
                               "--config", str(cfg)]).exit_code == 0
