...
def _r(score, gated=False):
    return CoherenceResult(score=score, gated=gated, questions=(), reasons=() if not gated else ("gate",))
...
def _qr(qid, score, confidence, max_score=2.0, subject=None):
    return QuestionResult(id=qid, kind="mission", score=score, max_score=max_score,
                          weight=1.0, is_gate=False, gate_tripped=False,
                          evidence="e", confidence=confidence, subject=subject)


def _with_q(score, questions, key=None):
    return CoherenceResult(score=score, gated=False, questions=tuple(questions),
                           reasons=(), artifact_key=key)
...
def test_solid_flip_on_one_question_is_a_localized_hard_fail():
    base = record_baseline([_with_q(0.9, [_qr("P1", 2.0, 1.0)], _KEY_A)],
...
def test_borderline_flip_does_not_hard_fail():
    ...
    base = record_baseline([_with_q(0.9, [_qr("P1", 2.0, 0.5)], _KEY_A)],
                           rubric_version="cdt-1")
...
def test_removing_an_artifact_from_the_suite_does_not_shift_layer_one_pairing():
    ...
    base = record_baseline([_with_q(0.9, [_qr("P1", 2.0, 1.0)], _KEY_A),
                            _with_q(0.9, [_qr("P1", 0.0, 1.0)], _KEY_B)],
                           rubric_version="cdt-1")
...
def test_a_genuine_flip_is_still_caught_when_the_suite_is_reordered():
    ...
    base = record_baseline([_with_q(0.9, [_qr("P1", 2.0, 1.0)], _KEY_A),
                            _with_q(0.9, [_qr("P1", 2.0, 1.0)], _KEY_B)],
                           rubric_version="cdt-1")
    rep = gate([_with_q(0.9, [_qr("P1", 0.0, 1.0)], _KEY_B),      # B flipped, now first
                _with_q(0.9, [_qr("P1", 2.0, 1.0)], _KEY_A)],
               baseline=base, threshold=0.85)
    assert rep.passed is False
    assert any("P1" in hf and "[0]" in hf for hf in rep.hard_fails)
...
def test_an_artifact_with_no_baseline_row_is_skipped_not_failed():
    ...
    base = record_baseline([_with_q(0.9, [_qr("P1", 2.0, 1.0)], _KEY_A)],
                           rubric_version="cdt-1")
    rep = gate([_with_q(0.9, [_qr("P1", 2.0, 1.0)], _KEY_A),
...
def test_reordered_principles_do_not_produce_a_false_flip():
    ...
    base = record_baseline(
        [_with_q(0.9, [_qr("P1", 2.0, 1.0, subject="Be fair")], _KEY_A)],
        rubric_version="cdt-1")
    rep = gate([_with_q(0.9, [_qr("P1", 0.0, 1.0, subject="Handle refunds kindly")], _KEY_A)],
               baseline=base, threshold=0.85)
...
def test_a_flip_under_the_same_principle_still_hard_fails():
    ...
    base = record_baseline(
        [_with_q(0.9, [_qr("P1", 2.0, 1.0, subject="Be fair")], _KEY_A)],
        rubric_version="cdt-1")
    rep = gate([_with_q(0.9, [_qr("P1", 0.0, 1.0, subject="Be fair")], _KEY_A)],
               baseline=base, threshold=0.85)
    assert rep.passed is False
...
def test_a_drop_past_tolerance_is_still_absorbed_by_real_variance():
    ...
    base = record_baseline([_r(s) for s in (1.0, 0.8, 1.0, 0.8, 0.9)],
                           rubric_version="cdt-1")           # mean 0.900, SE 0.045
    rep = gate([_r(s) for s in (0.95, 0.75, 0.95, 0.75, 0.85)],
               baseline=base, threshold=0.5)                 # mean 0.850, SE 0.045
    assert rep.drift is None and rep.passed is True
...
def test_a_drop_past_the_noise_band_is_still_absorbed_by_tolerance():
    ...
    base = record_baseline([_r(s) for s in (0.901, 0.899, 0.900, 0.901, 0.899)],
                           rubric_version="cdt-1")           # mean 0.900
    rep = gate([_r(s) for s in (0.886, 0.884, 0.885, 0.886, 0.884)],
               baseline=base, threshold=0.5)                 # mean 0.885
    assert rep.drift is None and rep.passed is True

    tight = gate([_r(s) for s in (0.886, 0.884, 0.885, 0.886, 0.884)],
                 baseline=base, threshold=0.5, tolerance=0.005)
...
def test_keys_present_but_all_none_is_treated_as_old_format():
    ...
    base = record_baseline([_with_q(0.9, [_qr("P1", 2.0, 1.0)], _KEY_A)],
                           rubric_version="cdt-1")
    base.artifact_keys = [None]
    rep = gate([_with_q(0.9, [_qr("P1", 0.0, 1.0)], _KEY_A)],
               baseline=base, threshold=0.85)
    assert rep.hard_fails == []
    assert any("re-accept" in n for n in rep.notes)
