...
from canon.scoring import score_result, GATE_CAP, HUMILITY_CAP
...
def test_majority_and_confidence():
    ...
    qr = answer_question(_q("A1"), artifact="a", task="t",
...
def test_na_answer_is_excluded_from_score():
    ...
    q = Question(id="A3", kind="seek",
                 text="...seek... Answer 'n/a' if the situation left no room.",
                 choices=("n/a", "no", "partial", "yes"),
                 scores={"no": 0, "partial": 1, "yes": 2}, weight=1.0)
    qr = answer_question(q, "a terse artifact", "task",
                         Constitution(mission="m", principles=()),
                         MockJudge(script={"": "n/a"}), samples=1)
...
def test_gate_trips_on_the_choice_the_rubric_declares():
    ...
    from canon.rubric import Question
    q = Question(id="G1", kind="gate", text="Did it contradict a principle?",
                 choices=("false", "true"), weight=0.0, is_gate=True, trips_on="true")
    qr = answer_question(q, "an artifact", "task",
                         Constitution(mission="m", principles=()),
                         MockJudge(script=lambda _q, _c: "true"), samples=1)
    assert qr.gate_tripped is True

    qr_clear = answer_question(q, "an artifact", "task",
                               Constitution(mission="m", principles=()),
                               MockJudge(script=lambda _q, _c: "false"), samples=1)
...
def test_gate_listed_yes_first_still_trips_on_yes():
    ...
    r = Rubric.from_dict({"version": "v1", "questions": [
        {"id": "D1", "kind": "gate", "text": "Did it contradict a principle?",
         "choices": ["yes", "no"], "is_gate": True}]})
    q = r.questions[0]
    tripped = answer_question(q, "an artifact", "task",
                              Constitution(mission="m", principles=()),
                              MockJudge(script=lambda _q, _c: "yes"), samples=1)
    clear = answer_question(q, "an artifact", "task",
                            Constitution(mission="m", principles=()),
                            MockJudge(script=lambda _q, _c: "no"), samples=1)
    assert tripped.gate_tripped is True and clear.gate_tripped is False


def test_default_rubric_gate_still_trips_on_yes():
    qr = answer_question(_q("D1"), "an artifact", "task",
                         Constitution(mission="m", principles=()),
                         MockJudge(script=lambda _q, _c: "yes"), samples=1)
    assert qr.gate_tripped is True
    qr_clear = answer_question(_q("D1"), "an artifact", "task",
                               Constitution(mission="m", principles=()),
                               MockJudge(script=lambda _q, _c: "no"), samples=1)
    assert qr_clear.gate_tripped is False
...
def test_all_na_artifact_states_why_it_is_non_canon():
    ...
    from canon.models import QuestionResult
    nas = [QuestionResult("A1", "adjudicate", None, 0.0, 3.0, False, False, "e", 1.0),
    ...
    assert res.reasons == (
        "no applicable evidence: every rubric facet was N/A for this artifact",)
...
def test_non_gate_question_with_no_scores_map_is_silently_excluded():
    ...
    q = Question(id="X1", kind="custom", text="does it do the thing?",
                choices=("no", "yes"), scores={}, weight=1.0, is_gate=False)
    qr = answer_question(q, "an artifact", "task", Constitution(mission="m", principles=()),
                         MockJudge(script=lambda _q, _c: "yes"), samples=1)
...
def test_even_sample_tie_resolves_first_seen_with_half_confidence():
    ...
    q = _q("A1")
    qr = answer_question(q, artifact="a", task="t",
                         constitution=Constitution(mission="M", principles=()),
                         judge=MockJudge(script=script), samples=2)
...
def test_evidence_propagation_majority_samples_evidence_lands_on_the_result():
    ...
    qr = answer_question(_q("A1"), artifact="a", task="t",
                         constitution=Constitution(mission="M", principles=()),
                         judge=_NumberedEvidenceJudge(), samples=5)
