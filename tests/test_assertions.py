...
from canon.assertions import task_is_coherent, criteria_covered
...
CON = Constitution(mission="Serve borrowers well",
                   principles=("Be fair", "Be transparent"))
...
def test_task_is_coherent_ambiguous_case_is_not_distinguishable_from_contradiction():
    ...
    with pytest.raises(AssertionError) as exc:
        task_is_coherent(CON, goal="ambiguous goal", criteria=["do something vague"],
                         judge=judge, samples=1)
...
def test_task_is_coherent_passes_on_a_clean_task():
    ...
    res = task_is_coherent(CON, goal="Approve loans fairly",
...
def test_task_is_coherent_fails_when_it_conflicts_with_a_principle():
    con = Constitution(mission="Serve borrowers well",
                       principles=("Never discriminate by protected class",))
    judge = _rule_judge([
        ("in play for THIS decision", "relevant", "e"),
        ("SERVE or VIOLATE", "violates", "e"),
        ("EITHER true", "yes", "e"),
    ...
    with pytest.raises(AssertionError, match="non-canon"):
        task_is_coherent(con, goal="Speed up approvals",
...
def test_criteria_covered_fails_and_names_the_missing_criterion():
    ...
    criteria = ["Verify affordability before approval",
    ...
    with pytest.raises(AssertionError) as exc:
        criteria_covered("Approval letter with affordability check but no "
                         "repayment schedule shown.",
                         goal="Approve responsibly", criteria=criteria,
                         constitution=CON, judge=judge, threshold=0.92, samples=1)
...
def test_criteria_covered_passes_when_all_criteria_are_met():
    ...
    res = criteria_covered("Approval letter with affordability check and full "
                           "repayment schedule.",
                           goal="Approve responsibly", criteria=criteria,
                           constitution=CON, judge=judge, samples=1)
    assert res.score >= 0.85 and not res.gated
...
def test_lending_use_case_task_then_result_are_both_checked():
    ...
    criteria = ["Verify income and existing debt before approval",
    ...
    task_res = task_is_coherent(LENDING_CON, goal=goal, criteria=criteria,
                                judge=_ScriptedJudge(task_judge_fn), samples=1)
    ...
    result_res = criteria_covered(result_artifact, goal=goal, criteria=criteria,
                                  constitution=LENDING_CON,
                                  judge=_ScriptedJudge(result_judge_fn), samples=1)
