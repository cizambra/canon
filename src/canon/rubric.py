    ...
    def from_dict(cls, data: dict) -> Rubric:
        ...
        if not any(q.is_gate for q in qs):
            warnings.warn(
                ...
                "outright, only the graded score applies",
                UserWarning,
                stacklevel=2,
            )
...
def _question_from_dict(q: dict) -> Question:
    ...
    for key, value in scores.items():
        ...
        if key not in choices:
            raise RubricError(
                f"question {qid!r}: score key {key!r} is not one of its choices {tuple(choices)}"
            )
    ...
    if trips_on is not None and not isinstance(trips_on, str):
        raise RubricError(
            f"question {qid!r}: 'trips_on' must be a string naming one of "
...
def relevant_questions(
    questions: list[Question], artifact: str, task: str, judge: Judge, report_excluded: bool = False
):
