...
def _question_flips(
    current: list[CoherenceResult], baseline: Baseline
    ...
    for key, rows in zip(baseline.artifact_keys or [], baseline.questions or [], strict=True):
        if key is not None:
    ...
    for i, result in enumerate(current):
        ...
        for q in result.questions:
            ...
            if row.get("subject") != q.subject:
                ...
                note = f"constitution changed under question {q.id}; re-accept to re-baseline"
            ...
            if was is None or now is None:
                continue
            if (
                was >= SOLIDLY_SATISFIED
                and row.get("confidence", 0.0) >= SOLID_CONFIDENCE
                and now <= SOLIDLY_VIOLATED
                and q.confidence >= SOLID_CONFIDENCE
            ):
                fails.append(
...
def gate(
    ...
    if baseline is not None:
        if (
            baseline.questions is None
            or baseline.artifact_keys is None
            ...
            notes.append(
                "baseline has no per-question record keyed by artifact, so "
            ...
            notes.append(
                "baseline artifact keys and question rows disagree in length, "
    ...
    if baseline and baseline.scores:
        ...
        if drop > max(DRIFT_Z * combined, tolerance):
            drift = (
                f"coherence regressed vs accepted baseline: "
    ...
    return GateReport(
        passed=not hard_fails and drift is None,
        hard_fails=hard_fails,
        drift=drift,
        mean=round(mean_cur, 4),
