...
from canon.config import Settings, discover_config_file
from canon.config import set_judge as _set_judge
...
def _metric(constitution_path: str, threshold: float) -> CoherenceMetric:
    ...
    return CoherenceMetric(
        constitution, threshold=threshold, judge=judge, samples=1 if judge else 5
    )
...
def set_judge(provider: str = typer.Option(...), model: str = typer.Option(...)):
    ...
    if source is not None and source.name == "pyproject.toml":
        ...
        typer.echo(
            "error: config lives in pyproject.toml [tool.canon]; set "
            "judge_model there (set-judge manages canon.yaml only)",
            err=True,
        )
...
@app.command()
def accept(
    suite: str = typer.Option("suite.yaml"),
    ...
    try:
        ...
        results = run_suite(suite, _metric(constitution or str(s.constitution_file), s.threshold))
...
def check(
    ...
    if rep.passed:
        typer.echo(
            f"PASS — mean coherence {rep.mean}, pass-rate {rep.pass_rate} "
